...
#' @export
teach_test <- function(testobj, export = FALSE, ...) {
  ...
  if (grepl("One", method, fixed = TRUE)) {
    df_combined <- eval(parse(text = data_name))
    ...
    if (!is.null(checkform)) { # = when input is valid
      ...
      # das was im if-Block steht, funktioniert, wenn AV und UV direkt im environment geladen sind (z.B. als zwei separate data vectors)
      # das, was im else if Block steht, braucht man, wenn AV und UV in einem data frame sind und R daher nicht direkt auf die Daten zugreifen kann
        # -> in dem Fall müsste man in der teach-Funktion als user noch das Zusatzargument "data = " hinzufügen
    }
    else{
    ...
    if (is.null(ncol(testobj[["lm"]][["fitted.values"]]))) { # checks structure of fitted values (varies between within and between design)
      ...
      meanvals$meanvals <- round(meanvals$meanvals, 12)
      meanvals <- meanvals %>%
    ...
    if (list_within == 0) {
      ...
      Notes <- c(
        "",  paste0("Supplementary Information: The group sizes are ", group_sizes)
      )
      if (heterosk == T) Notes <- c(Notes, "- Variances might be unequal")
    ...
    if (grepl("*", model_formula, fixed = TRUE) | grepl(":", model_formula, fixed = TRUE)){
      ...
      Calculation <- c(" ",
                       ...
                       "p1 = 2*(1-pt(abs(t_emp1), df))",
                       "p2 = 2*(1-pt(abs(t_emp2), df))",
                       "p3 = 2*(1-pt(abs(t_emp3), df))")
    ...
    # multiple linear regression
      ...
      Calculation <- c(" ",
                       ...
                       "p1 = 2*(1-pt(abs(t_emp1), df))",
                       "p2 = 2*(1-pt(abs(t_emp2), df))",
                       "b1s = (rx1y-rx2y*rxx)/(1-rxx^2)",
                       "b2s = (rx2y-rx1y*rxx)/(1-rxx^2)")
